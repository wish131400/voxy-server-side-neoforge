...
public class StageExactGridBench {
 static boolean vss;
 ...
 static long query(long h,int x,int z,int side,int step,boolean single) {
  ...
  if(vss && fullChunks) {
   ...
   for(var group:groups.values()) for(int offset=0;offset<group.size();offset+=64) {
    ...
    long start=System.nanoTime();
    int wrote=RustWorldgenBackend.surfacePoints(h,input,temporary,n);
    ns+=System.nanoTime()-start;
   ...
   for(int offset=0;offset<side*side;) {
    ...
    long start=System.nanoTime();
    int wrote=RustWorldgenBackend.surfacePoints(h,input,out,n);
    ns+=System.nanoTime()-start;
 ...
 public static void main(String[] args) throws Exception {
  ...
  String[] names={"single_scattered","batch8_step1","batch8_step4","tile66_step1","tile66_chunked","batch8_fullchunk","batch8_step64","tile66_warm"};
  int[] sides={16,8,8,66,66,8,8,66}, steps={64,1,4,1,1,1,64,1}, tiles={1,16,16,1,1,16,4,1};
