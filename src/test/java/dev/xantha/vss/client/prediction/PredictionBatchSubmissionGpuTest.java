...
/** Submission prototype only: shared packed payload, owner table and array masks.
...
class PredictionBatchSubmissionGpuTest {
    ...
    @Test void sharedPageMatchesIndividualTilesAndMeasuresSubmissionCost() {
        ...
        try {
            ...
            try(var old=GlProgram.link("batch_reference",VERTEX,FRAGMENT);
                ...
                long[][] cpu=new long[2][60],gpu=new long[2][60];byte[][] pixels=new byte[2][];
                for(int frame=-30;frame<60;frame++) for(int mode:frame%2==0?new int[]{0,1}:new int[]{1,0}) {
                    ...
                    if(mode==0) {
                        ...
                        batch.use();bind(0,GL_TEXTURE_BUFFER,payload);bind(1,GL_TEXTURE_2D_ARRAY,mask);
                    ...
                    if(frame==0 || frame==30) {
                        ...
                        if(mode==(frame%2==0?1:0)) assertArrayEquals(pixels[0],pixels[1],"same payload/ownership must produce identical pixels");
                    }
                    if(frame==20 && mode==1) {
                ...
                System.out.printf(Locale.ROOT,"BATCH_PROTOTYPE tiles=512 CPU_individual_ms=%.3f CPU_shared_ms=%.3f GPU_individual_ms=%.3f GPU_shared_ms=%.3f calls=512->1 bindings=1024->4 extraOwnerBytesPerQuad=4 (submission prototype; not production FPS)%n",
                        cpu[0][30]/1e6,cpu[1][30]/1e6,gpu[0][30]/1e6,gpu[1][30]/1e6);
